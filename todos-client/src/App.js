...
class App extends React.Component {
    ...
    constructor(props) {
        super(props);
        this.state = {todoLists: [], newListName: '', 
        updatedListName: '', todoItemAdders: []};
...
class TodoFilter extends React.Component {
    ...
    isActive(value){
        return 'btn '+((value===this.props.filter.Status) ? 'btn-primary':'default');
    }
                ...
                <div className="row">
                    <div className="col-xs-7">
                        <div id="todo-filter" className="input-group">
                            <span className="input-group-btn">
                                ...
                                <input  type="search" className="form-control" ref='filter' onChange={onSearch1} placeholder="Search the items" ></input>
                        </div>
                    </div>
                    <div className="col-xs-5">
                        <ul className="nav nav-pills todo-filter">
                            ...
                            <li><Button onClick={onFilter1} className={this.isActive('SHOW_INCOMPLETE')} value="SHOW_INCOMPLETE">Incomplete</Button></li>
                             <li><Button onClick={onFilter1} className={this.isActive('SHOW_INCOMPLETE')} value="SHOW_COMPLETE">Complete</Button></li>
                        </ul>
                    </div>
                </div>
        ); 
...
class TodoList extends React.Component {
    ...
    handleDeleteItem(listId, itemId) {
        fetch('http://localhost:8080/lists/' + listId + '/items/' + itemId, {
            method: 'DELETE',
        }).then(response => {
            this.setState((prevState, props) => {
    ...
    filterItem(event){
        this.state.filter[0].Status = event.target.value;
        this.setState({
    ...
    searchItem(event){
        this.state.filter[0].keyword = event.target.value;
        this.setState({
            filter: this.state.filter
        });
    }
    ...
    render() {
        ...
        let queryText = this.state.filter[0].keyword;
        let status = this.state.filter[0].Status;
        let items = this.state.items.map((item) => {
            ...
            } else if(queryText | item.name.toLowerCase().indexOf(queryText)!= -1){
        ...
        return (
            <div>
                <form onSubmit={this.handleSubmit}>
                    <FormGroup>
                        <TodoFilter onFilter = {this.filterItem} onSearch = {this.searchItem} 
                            filter={this.state.filter}/>
                        <InputGroup>
                            <FormControl
                                type="text"
